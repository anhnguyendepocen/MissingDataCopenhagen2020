...
Of the `r dim(nhanesMort)[1]` observations in the dataset, `r dim(nhanesMort)[1]-sum(complete.cases(nhanesMort))` have been omitted because at least one of the variables involved in the regression has a missing value. The impact of this is that the standard errors from the CCA are narrower than what they would have been with complete data, and confidence intervals are wider. The estimates may also be biased.
...
```{r}
summary(glm(is.na(ALQ150)~dead10+gender+age+ethnicity+sbp+waist_circum+weight+total_chol+hdl, data=nhanesMort, family="binomial"))
```
